...
3) Build MATLAB MEX binding for BoomerAMG for magnetic_potential_problem_2D.
...
HYPRE_BUILD_DIR="${HYPRE_BUILD_DIR:-$REPO_DIR/third_party/hypre-build-openmp}"
HYPRE_INSTALL_DIR="${HYPRE_INSTALL_DIR:-$REPO_DIR/third_party/hypre-openmp}"
...
if [[ $DO_MEX -eq 1 ]]; then
  echo "MEX source built via: magnetic_potential_problem_2D/+LINEAR_SOLVERS/build_hypre_boomeramg_mex.m"
fi
