...
%                    mapping. For 3D, elasticity near-null-space vectors are also
%                    built automatically.
...
if any(solver_type == ["DFGMRES_AGMG", "DCG_AGMG"]) && is_agmg_present == 0
    warning('AGMG not found or unusable. Switching to %s.', char(replace(solver_type, "AGMG", "ICHOL")));
    solver_type = replace(solver_type, "AGMG", "ICHOL");
end
...
switch solver_type
    ...
    case "DCG_ICHOL"
        preconditioner_builder = @(A) LINEAR_SOLVERS.diag_prec_ICHOL(A, Q);
        linear_system_solver = LINEAR_SOLVERS.DCG(preconditioner_builder, ...
            linear_solver_tolerance, linear_solver_maxit, ...
            deflation_basis_tolerance, linear_solver_printing);
    ...
    case {"DCG_HYPRE_BOOMERAMG", "DCG_BOOMERAMG"}
        preconditioner_builder = local_build_boomer_builder(Q, coord, boomeramg_opts);
        linear_system_solver = LINEAR_SOLVERS.DCG(preconditioner_builder, ...
            linear_solver_tolerance, linear_solver_maxit, ...
            deflation_basis_tolerance, linear_solver_printing);
...
opts = opts_in;
custom_hypre_root = [];
if isfield(opts, 'hypre_root')
    custom_hypre_root = char(string(opts.hypre_root));
    opts = rmfield(opts, 'hypre_root');
end

if exist('hypre_boomeramg_mex', 'file') ~= 3
    this_dir = fileparts(mfilename('fullpath'));
    if isfile(fullfile(this_dir, 'build_hypre_boomeramg_mex.m'))
        LINEAR_SOLVERS.build_hypre_boomeramg_mex(custom_hypre_root);
    else
...
auto_dof_func = [];
if isvector(Q) && ~isempty(coord) && numel(Q) == size(coord, 2)
    % Scalar unknowns with nodal coordinates: one DOF per node.
    n_unknown = numel(Q(:));
    active = find(Q(:));
    active = active(:);
    q_idx = (1:numel(active))';
    auto_dof_func = zeros(size(q_idx));
    n_components = 1;
...
elseif ~isempty(coord) && ~isempty(Q) && isequal(size(Q), size(coord)) && ...
    ...
    % Vector-valued unknowns (2D/3D), component-wise HYPRE mapping.
