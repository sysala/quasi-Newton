function [U, it, crit_hist, omega_hist, itcg, timed_out] = newton_quasi1(U_ini, WEIGHT, K_fix, B, f, heter_int, alpha, beta, linear_system_solver, max_runtime_seconds)
%--------------------------------------------------------------------------
...
if use_iterative_solver
    linear_system_solver.setup_preconditioner(K);
end
...
while true
    if toc(method_timer) >= max_runtime_seconds
        ...
        fprintf('     Quasi-Newton 1 timed out after %.2f s\n', max_runtime_seconds)
        break
    end
    ...
    % tangential stiffness matrix. Update only when DS changes materially.
    if max(abs(DS1 - DS_old) ./ max(abs(DS1), eps)) > 1e-1
        vD = repmat(WEIGHT, 2, 1) .* DS1;
    ...
    if it == it_max
        omega_hist = omega_hist(1:it - 1);
        fprintf('     Quasi-Newton 1 converges slowly: stopping criterion=%e  ', criterion)
        fprintf('\n');
        break
    end
    ...
    if use_iterative_solver
        ...
        linear_system_solver.A_orthogonalize(K);
        [dU, iter] = linear_system_solver.solve(K, b);
        itcg = itcg + iter;
